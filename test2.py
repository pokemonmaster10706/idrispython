...
def customer():
    ...
    def search_customer():
        ...
        if search.isdigit():
            cur_search.execute('select * from Customers where ISBNo = '+search)
        else:
            cur_search.execute('select * from Customers where Book_Name = "'+search+'"')
...
def return_book():
    ...
    cur_rtrn = sqlcon.cursor()

    custid = int(input('Enter customer ID: '))
    isbno = int(input('Enter ISBNo of the book: '))
    cur_rtrn.execute('select * from issue where Cust_id = %s and isbno = %s'%(custid,isbno))
    try:
        issue_date = cur_rtrn.fetchall()[0][1]
    except:
        print('There was no record of this book being issued by this customer')
    diff = datediff(issue_date)
    if diff > 14:
        fine = (diff - 14) * 15
    else:
        fine = 0
    paid = input('Has the Customer Paid the fine amount?(y/n): ')

    cur_rtrn.execute('insert into returns(isbno,cust_id,fine_amount,paid) values(%s,%s,%s,"%s")'%(isbno,custid,fine,paid))
    sqlcon.commit()
    cur_rtrn.execute('select * from books where ISBNo = %s'%(isbno,))
    nbook = cur_rtrn.fetchall()[0][7]
    cur_rtrn.execute('update books set no_of_copies = "%s" where isbno = "%s"'%(nbook+1,isbno))
    sqlcon.commit()
    print('\n--Book Returned--\n')
    cur_rtrn.close()

if __name__ == '__main__':
    sqldb = msconn.connect(host = 'localhost', user = 'root', passwd = 'idris7')
    check_database()
    main()
    sqlcon.close()
